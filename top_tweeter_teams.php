...
if ($min > 0) {
    echo "<a type='button' class='btn btn-default pull-left' href='./top_tweeter_teams.php?sort=$sort&min=$prev_min'>
                    <span class='glyphicon glyphicon-chevron-left'></span> Previous 
                </a>";
}

echo "
                <a type='button' class='btn btn-default pull-right' href='./top_tweeter_teams.php?sort=$sort&min=$next_min'>
...
$result = query_boinc_db("SELECT id, name, valid_tweets, total_tweets, country, create_time FROM team ORDER BY $sort_by LIMIT $min, 20");
...
$navbar_template = file_get_contents($cwd[__FILE__] . "/templates/top_tweeter_teams.html");
